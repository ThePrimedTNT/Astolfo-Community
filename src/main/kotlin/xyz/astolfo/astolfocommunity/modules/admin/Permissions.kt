...
//    }
//
//    suspend fun CommandContext.parseScope(args: ArgsIterator, data: PermissionData, includePermission: Boolean): Boolean {
...
//                if (it.isBlank()) chatInput("Input a path").execute() ?: return false
//                else it
//            }
//        }
//
...
//
//    command("grant") {
//        usage("channel <channel> <path> <role>", "guild <path> <role>")
//        description("Grants path to a role in a guild/channel")
//        action {
//            parseScope(args.argsIterator(), it, true)
//        }
...
//                }
//                messageAction(embed("You have granted the path **$permission** to the role **${role.name.let { if (it.startsWith("@")) it.substring(1) else it }}** under the ${if (channel == null) "guild" else "channel ${channel.asMention}"} scope")).queue()
//            }
//        }
//    }
//
//    command("deny") {
//        usage("channel <channel> <path> <role>", "guild <path> <role>")
//        description("Denies path to a role in a guild/channel")
//        stageActions<PermissionData> {
//            action({ parseScope(args.argsIterator(), it, true) })
//            basicAction {
//                val (role, channel, permission) = Triple(it.role, it.textChannel, it.permission)
...
//                }
//                messageAction(embed("You have denied the path **$permission** to the role **${role.name.let { if (it.startsWith("@")) it.substring(1) else it }}** under the ${if (channel == null) "guild" else "channel ${channel.asMention}"} scope")).queue()
//            }
//        }
//    }
//
//    command("reset") {
//        usage("channel <channel> <path> <role>", "guild <path> <role>")
//        description("Defaults path to a role in a guild/channel")
//        stageActions<PermissionData> {
//            action({ parseScope(args.argsIterator(), it, true) })
//            basicAction {
//                val (role, channel, permission) = Triple(it.role, it.textChannel, it.permission)
...
//                }
//                messageAction(embed("You have defaulted the path **$permission** to the role **${role.name.let { if (it.startsWith("@")) it.substring(1) else it }}** under the ${if (channel == null) "guild" else "channel ${channel.asMention}"} scope")).queue()
//            }
//        }
//    }
//
...
//}
//
//private suspend inline fun <E> CommandContext.withPermissions(block: (MutableMap<PermissionSetting, Boolean>) -> E): E = withGuildSettings {
